...
all_classes.forEach(element => {
    let ret = element.matcher+"(";
    let params = "";
    if(typeof element.arguments!=="undefined"){
        params +=  "<";
        let k = 0;
        for (var key in element.arguments[0]){
            if(k)params +=  ",";
            k++;
            params += key+":"+ element.arguments[0][key];
...
params +=  ">";
    ...
    let kk=0;
        for (var key in element.styles){
            ct+= key+":"+ element.styles[key]+";";
        if(kk==0){    
  let result = String(element.styles[key]).match(/\$\d/g);
  if(result){
